use crate::components::{
    MipsMmu, MMU_ADDRESS_IN_ID, MMU_IO_REG_SEL_OUT, MMU_IO_RE_OUT, MMU_IO_WE_OUT,
    MMU_MEM_ADDRESS_OUT_ID, MMU_MEM_RE_OUT, MMU_MEM_WE_OUT, MMU_READ_ENABLE_IN,
    MMU_TIMER_ADDRESS_OUT, MMU_TIMER_RE_OUT, MMU_TIMER_WE_OUT, MMU_WRITE_ENABLE_IN,
};
...
impl EguiComponent for MipsMmu {
    ...
    fn ports_location(&self) -> Vec<(syncrim::common::Id, Pos2)> {
        ...
        vec![
            ...
            // TODO CP0 ports
            (MMU_MEM_ADDRESS_OUT_ID.to_string(), out_pos(3) + own_pos),
            (MMU_MEM_RE_OUT.to_string(), out_pos(4) + own_pos),
            (MMU_MEM_WE_OUT.to_string(), out_pos(5) + own_pos),
            (MMU_TIMER_ADDRESS_OUT.to_string(), out_pos(6) + own_pos),
            (MMU_TIMER_RE_OUT.to_string(), out_pos(7) + own_pos),
            (MMU_TIMER_WE_OUT.to_string(), out_pos(8) + own_pos),
            (MMU_IO_REG_SEL_OUT.to_string(), out_pos(9) + own_pos),
            (MMU_IO_RE_OUT.to_string(), out_pos(10) + own_pos),
            (MMU_IO_WE_OUT.to_string(), out_pos(11) + own_pos),
        ]
    }

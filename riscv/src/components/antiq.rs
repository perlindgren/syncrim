...
use std::{cell::RefCell, cmp::Reverse, collections::HashMap, convert::Into};
use syncrim::common::{
...
pub struct Antiq {
    ...
    pub mmio: RefCell<HashMap<usize, u8>>,
}
...
impl Component for Antiq {
    ...
    fn clock(&self, simulator: &mut Simulator) -> Result<(), Condition> {
        ...
        let we: u32 = simulator.get_input_value(&self.we).try_into().unwrap_or(0);
        let d_in: u32 = simulator.get_input_value(&self.d_in).try_into().unwrap_or(0);
        let addr: u32 = simulator.get_input_value(&self.addr).try_into().unwrap_or(0);
        let size: u32 = simulator.get_input_value(&self.size).try_into().unwrap_or(0);
        ...
        if we == 2 && self.within_range(addr) {
            match addr - self.RANGE_LOW {
                0 => {
                    //relative time maybe ignore this for now, needs signal to antiq to trigger
                    //adding sysclk
                ...
                8 => {
                    //drop input/push output
                    drop_id = d_in;
                    drop = true;
                }
                int => {
                    //do nothing, we've checked that addr is within range
                    trace!("addr diff{}", int)
                }
            }
        }
        if we == 1 && self.within_range(addr) {
            trace!("read addr:{}", (addr - self.RANGE_LOW));
            simulator.set_out_value(
                &self.id,
                "data_o",
                self.read(
                    (addr - self.RANGE_LOW) as usize,
                    size as usize,
                    false,
                    false,
                ),
        ...
        if push {
            ...
            self.write(
                ...
                size as usize,
                false,
                SignalValue::Data(sysclk),
            );
            trace!("write at addr:{}", (addr - self.RANGE_LOW));
            //simulator.set_out_value(&self.id, "data_o", sysclk)
        } else {
        ...
        if pop {
            ...
            simulator.set_out_value(&self.id, "int_id", 0);
        }

        if drop {
            queue.remove(&drop_id);
            simulator.set_out_value(&self.id, "drop_rdy_o", 1);
        } else {
            simulator.set_out_value(&self.id, "drop_rdy_o", 0);
        }
        if queue.len() == self.DEPTH as usize {
        ...
        if queue.is_empty() {
            ...
            simulator.set_out_value(&self.id, "peek_data_o", *queue.peek().unwrap().0);
            // infallible since queue
            // is not empty
        }
...
mod test {
    ...
    fn test_push() {
        let cs = ComponentStore {
            store: vec![
                Rc::new(ProbeOut::new("pop_i")),
                Rc::new(ProbeOut::new("drop_i")),
                Rc::new(ProbeOut::new("push_i")),
                Rc::new(ProbeOut::new("drop_id_i")),
                Rc::new(ProbeOut::new("push_id_i")),
        ...
        assert_eq!(
            simulator.get_input_value(peek_data_o),
            98.try_into().unwrap()
        );
        assert_eq!(simulator.get_input_value(overflow_o), 0.try_into().unwrap());
        assert_eq!(
            simulator.get_input_value(data_overflow_o),
            0.try_into().unwrap()
        );

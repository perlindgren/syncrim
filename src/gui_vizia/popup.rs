use crate::{common::Ports, gui_vizia::GuiData};
use vizia::prelude::*;
...
impl NewPopup {
    pub fn new(cx: &mut Context, id_ports: (String, Ports)) -> Handle<NewPopup> {
...
pub fn new_popup(cx: &mut Context, id_ports: (String, Ports)) {
    PopupData::default().build(cx);
    Popup::new(cx, PopupData::is_open, true, move |cx| {
        VStack::new(cx, |cx| {
            let (id, ports) = id_ports.clone();

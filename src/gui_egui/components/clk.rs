...
#[typetag::serde]
impl EguiComponent for MIPSCLK {
    fn render(
        &self,
        ui: &mut Ui,
        ...
        let r = rect_with_hover(rect, clip_rect, editor_mode, ui, self.id.clone(), |ui| {
            ...
            if let Some(s) = &simulator {
                ui.label({
                    let clk_r: Result<SignalUnsigned, String> =
                        s.get_input_value(&self.clk_in).try_into();
        ...
        properties_window(
            ...
            |ui| {
                ...
                clicked_dropdown |= input_selector(
                    ui,
                    &mut self.clk_in,
                    crate::components::CLK_IN_ID.to_string(),
                    id_ports,
                    self.id.clone(),
                );
                clicked_dropdown

            ...
            if let Some(s) = &simulator {
                ui.label({
                    let a_r: Result<SignalUnsigned, String> =
                        s.get_input_value(&self.instr_addr_in).try_into();

                    let b_r: Result<SignalUnsigned, String> =
                        s.get_input_value(&self.jump_addr_in).try_into();
        ...
        properties_window(
            ...
            |ui| {
                ...
                clicked_dropdown |= input_selector(
                    ui,
                    &mut self.instr_addr_in,
                    crate::components::MERGE_INSTR_ADDR_ID.to_string(),
                    id_ports,
                    self.id.clone(),
                );
                clicked_dropdown |= input_selector(
                    ui,
                    &mut self.jump_addr_in,
                    crate::components::MERGE_JUMP_ADDR_ID.to_string(),
                    id_ports,
                    self.id.clone(),
                );
                clicked_dropdown
